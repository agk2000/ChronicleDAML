...
class AllAuthors(Resource):
    ...
    def get(self):
        ...
        r = requests.get('https://dtc.ceo.getsnworks.com/v3/author', headers = header)
        count = r.json().get('total_pages')
        item_list =  []
        for i in range(1, count+1):
            payload = {'page':str(i)}
            r = requests.get('https://dtc.ceo.getsnworks.com/v3/author', headers = header, params=payload)
