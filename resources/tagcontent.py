...
class TagContent(Resource):
    ...
    def get(self, name):
        ...
        r = requests.get('https://dtc.ceo.getsnworks.com/v3/search?tag=' + name + '&type=content&page=1', headers = header)
        count = r.json().get('total_pages')
        item_list =  []
        for i in range(1, count+1):
            r = requests.get('https://dtc.ceo.getsnworks.com/v3/search?tag=' + name + '&type=content&page='+str(i), headers = header)
            newlist = r.json().get('items', [])
            item_list.extend(newlist)
        return item_list

class TagContentClean(Resource):
    @jwt_required()
    def get(self, name):
        init_response = TagContent.get(self, name)
